...
public class New_zad1 {
	...
	static String encript(char[][] matrix, String text) {
		...
		for (int index = 0; index < text.length(); index += 2) {
			...
			if (text.charAt(index) == 'Z' || text.charAt(index + 1) == 'Z') {
				encriptedText = encriptedText + text.charAt(index) + text.charAt(index + 1);
				continue;
			}
	...
	static String decript(char[][] matrix, String text) {
		...
		for (int index = 0; index < text.length(); index += 2) {
			...
			if (text.charAt(index) == 'Z' || text.charAt(index + 1) == 'Z') {
				decriptedText = decriptedText + text.charAt(index) + text.charAt(index + 1);
				continue;
			}			
			if (row1 == row2 && col1 != col2) {
				if (col1 == 0) {
